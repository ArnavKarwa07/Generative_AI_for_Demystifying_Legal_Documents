...
class AIService:
    ...
    def _identify_clause_type(self, text: str) -> str:
        ...
        if any(
            word in text_lower
            for word in ["confidential", "non-disclosure", "proprietary"]
        ):
            return "confidentiality"
        elif any(
            word in text_lower for word in ["payment", "fee", "compensation", "invoice"]
        ):
            return "payment"
        elif any(
            word in text_lower for word in ["terminate", "termination", "end", "expire"]
        ):
            return "termination"
        elif any(
            word in text_lower
            for word in ["indemnify", "indemnification", "liability", "damages"]
        ):
            return "indemnification"
        elif any(
            word in text_lower
            for word in ["intellectual property", "copyright", "patent", "trademark"]
        ):
            return "intellectual_property"
        elif any(
            word in text_lower
            for word in ["governing law", "jurisdiction", "dispute", "arbitration"]
        ):
    ...
    async def explain_clause(
        self, clause_text: str, explanation_type: str
    ...
    async def simulate_clause_change(
        self, original_clause: str, modified_clause: str
    ) -> SimulateClauseResponse:
    ...
    async def generate_alternatives(
        self, alternatives_request: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def analyze_document(
        ...
        """Analyze uploaded legal document and provide insights"""

        prompt = f"""
        Analyze this legal document and provide comprehensive insights:
        ...
        6. Missing clauses or provisions that should be considered
        7. Overall summary and assessment
        ...
        try:
            response = self.client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a senior legal analyst specializing in document review and analysis. Provide thorough, accurate analysis with actionable insights.",
                    },
                    {"role": "user", "content": prompt},
                ],
                model=self.model,
            ...
            return {
                ...
                "risk_level": self._assess_overall_risk(document_text),
                "key_clauses": self._identify_key_clauses(document_text),
                "recommendations": self._generate_recommendations(analysis_text),
                "summary": self._generate_summary(analysis_text),
                "word_count": len(document_text.split()),
                "analyzed_at": "2025-01-21",  # Would use actual timestamp
            }
    ...
    def _extract_document_type(self, text: str) -> str:
        ...
        if any(
            term in text_lower
            for term in ["employment agreement", "employment contract", "job offer"]
        ):
            ...
            term in text_lower
            for term in ["non-disclosure", "nda", "confidentiality agreement"]
        ):
            return "Non-Disclosure Agreement"
        elif any(
            term in text_lower
            for term in ["lease agreement", "rental agreement", "tenancy"]
        ):
            return "Lease Agreement"
        elif any(
            term in text_lower
            for term in ["purchase agreement", "sales contract", "buy-sell"]
        ):
            return "Purchase Agreement"
        elif any(
            term in text_lower for term in ["partnership agreement", "joint venture"]
        ):
            return "Partnership Agreement"
        else:
            return "General Legal Document"

    def _assess_overall_risk(self, text: str) -> str:
        """Assess overall risk level of document"""
    ...
    def _identify_key_clauses(self, text: str) -> List[str]:
        """Identify key clauses in the document"""
        clauses = []
        text_lower = text.lower()

        clause_indicators = {
            "Termination Clause": ["termination", "terminate", "end of agreement"],
            "Payment Terms": ["payment", "fee", "compensation", "invoice"],
            "Confidentiality": ["confidential", "non-disclosure", "proprietary"],
            "Liability": ["liable", "liability", "damages", "indemnification"],
            "Intellectual Property": ["intellectual property", "copyright", "patent"],
            "Governing Law": ["governing law", "jurisdiction", "disputes"],
        }
    ...
    def _generate_recommendations(self, analysis_text: str) -> List[str]:
        """Extract recommendations from analysis text"""
        # Simplified recommendation extraction
        recommendations = [
            "Review all clauses with legal counsel",
            "Ensure all parties understand their obligations",
            "Consider adding force majeure clause if missing",
            "Verify compliance with local laws and regulations",
        ]
        return recommendations

    def _generate_summary(self, analysis_text: str) -> str:
        """Generate a brief summary from analysis"""
        # This would normally use the analysis text to create a summary
        return "Document has been analyzed for legal compliance and risk factors. Please review the detailed analysis and recommendations above."

    async def simulate_changes(
        self, simulation_request: Dict[str, Any]
    ) -> Dict[str, Any]:
